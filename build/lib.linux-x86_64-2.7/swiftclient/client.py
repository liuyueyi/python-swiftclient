...
def get_auth_1_0(url, user, key, snet):
    ...
    #if ',' in resp.getheader('x-storage-url'):
    try:
        url = resp.getheader('x-storage-url').split(',')[0]
    except:
    ...
    # by wuzebang    
    return resp.getheader('x-storage-url'), resp.getheader('x-storage-token',
                               resp.getheader('x-auth-token')) 
    #, resp.getheader('x-attr-auth-token'), resp.getheader('x-user-group')
...
def get_object(url, token, container, name, http_conn=None,
    ...
    if resp.status < 200 or resp.status >= 300:
        ...
        http_log(('%s%s' % (url.replace(parsed.path, ''), path), method,),
                 {'headers': headers}, resp, body)
        # added by wuzebang
        print 'wuzebang, error'
        print resp.status, 'wuzebang'
        return 'error', 'error'
        #
        ...
        while self.attempts <= self.retries:
            ...
            try:
                if not self.url or not self.token:
                	# add by wuzbeang: self.attr_token
                    self.url, self.token = self.get_auth()
                    # wuzebang
                    ll = self.url.split(',')
                    self.url = ll[0]
                    del ll[0]
                    self.group_list = ','.join(ll)                    
                ...
                # add by wuzebang: self.attr_token
